...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
        ...
        elif module_def['type'] == 'depwiseconvolutional':
            bn = int(module_def['batch_normalize'])
            filters = int(module_def['filters'])
            kernel_size = int(module_def['size'])
            pad = (kernel_size - 1) // 2 if int(module_def['pad']) else 0
            modules.add_module('depthwiseconv_%d' % i, nn.Conv2d(in_channels=output_filters[-1],
            ...
            if bn:
                modules.add_module('batch_norm_%d' %
                                   i, nn.BatchNorm2d(output_filters[-1]))
            if module_def['activation'] == 'leaky':
                modules.add_module('leaky_%d' % i, nn.LeakyReLU(0.1))
            modules.add_module('pointconv_%d' % i, nn.Conv2d(
...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool','depwiseconvolutional']:
                x = module(x)
            elif mtype == 'route':
